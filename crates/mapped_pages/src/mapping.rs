...
impl<F: NodeFrameOps> MappedPages<F> {
    ...
    pub unsafe fn new_borrowed(
        ...
        assert!(
            page_count > 0,
            "MappedPages::new_borrowed: page_count 不能为 0"
        );
        Self {
            vaddr,
            page_count,
...
mod tests {
    ...
    fn new_borrowed_ownership() {
        let pt_ref = test_pt();
        // SAFETY: 测试环境下不会真正解引用这些地址
        let mp =
            unsafe { Mp::new_borrowed(pt_ref, VirtAddr::new(0x3_0000), 2, PteFlags::kernel_rw()) };
    ...
    fn into_permanent_marks_permanent() {
        ...
        // SAFETY: 测试环境下不会真正解引用这些地址
        let mp =
            unsafe { Mp::new_borrowed(pt_ref, VirtAddr::new(0x4_0000), 1, PteFlags::kernel_rw()) };
    ...
    fn as_type_reads_mapped_memory() {
        ...
        // SAFETY: buf 指向有效堆内存
        let mp = unsafe { Mp::new_borrowed(pt_ref, va, 1, PteFlags::kernel_rw()) };
    ...
    fn as_type_mut_writes_mapped_memory() {
        ...
        // SAFETY: va 指向 buf 内的页对齐地址，且页表中已有映射
        let mut mp = unsafe { Mp::new_borrowed(pt_ref, va, 1, PteFlags::kernel_rw()) };
