...
- **测试**:
  ...
  - 已补 RISC-V/AArch64 远端访问强证明：CPU0 改权限后 shootdown，CPU1 在 ack 后写目标 VA
    必须触发写权限异常。RISC-V 通过 store page fault 恢复钩子匹配 `stval/sepc`；AArch64
    通过写 data abort 恢复钩子匹配 `FAR_EL1/ELR_EL1`。
