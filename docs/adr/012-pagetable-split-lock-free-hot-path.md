...
**hot path 是 `OwnedPages::{new, set_flags, Drop}`**——这三条路径调用 `batch_update_pte → update_pte`，操作的是**已经存在的 PTE**（因为 boot 期 `identity_map_range` 已经建立了全量背景映射）。`update_pte` 本身不分配中间节点、不修改 `PageTable.nodes` 容器、不修改页表骨架——它只改单个叶 PTE 的 flags 字段。

**slow path（boot/MMIO）**：`identity_map_range` 通过 `create_pte → walk_create` 建立新 PTE，可能分配中间节点。
...
[ADR-007](007-eliminate-vma-and-dead-code.md) 确立 **"PTE 只建不删"**——SAS 全量映射模型下没有 `unmap` 路径，PTE 的生命周期是 "创建 (create_pte) → 修改 flags (update_pte) → 永远存在"。
...
虽然 SAS 消除了节点生命周期竞态，但 **权限变动的并发安全** 仍需论证——两个核能否同时修改同一页的 PTE flags？

答案是 **Rust 所有权模型在编译期排除了这种可能**：

1. **`AllocatedFrames` 不可 Clone/Copy**——每个物理帧只被一个 `OwnedPages` 持有（move 语义）
2. **`OwnedPages::set_flags(&mut self)`** 要求独占引用——同一个 `OwnedPages` 不会被并发修改
3. **`OwnedPages::drop(&mut self)`** 消费 self——不会与其他方法并发

因此，通过 `OwnedPages` API 的合法代码路径下，**同一 PTE 永远不会被并发写入**。无锁 `update_pte` 只需保证不同 PTE 的并发写入安全——`AtomicU64` per-entry 天然满足。

| 并发场景 | 保护机制 | 安全性 |
|---------|---------|--------|
| 不同页同时改权限 | AtomicU64 per-entry | 安全 |
...
- **场景 B**：CPU0 `update_pte` swap 新 flags，同时 CPU1 `walk_to_leaf` 读该 PTE
...
impl PageTable {
    ...
    /// 建新 PTE——仅此路径需要锁
    pub fn create_pte(&self, va: VirtAddr, pa: PhysAddr, flags: PteFlags) -> Result<(), PagingError> {
        ...
        // walk_create_and_write 内部遇到已存在的 PTE 时需按幂等/冲突语义处理，
        // 不得重复分配中间节点。
...
// 或直接替换 Table::write 为专用 swap 路径（见 update_pte）
```
...
- **现有 paging-test 需要适配**：调用 `PageTable::create_pte(&mut pt, ...)` 改为 `PageTable::create_pte(&pt, ...)`；test 中的 `let mut pt` 可以改为 `let pt`
- **新增多核并发测试**：在 `tests/paging-test/` 下新建 `concurrent.rs` 二进制——多 CPU 同时做 `update_pte` 与 `get_mapping`，验证无 torn read、无 panic
