...
static constexpr const uint32_t kCom1 = 0x3F8;  // COM1端口基址
```
...
class Pl011 {
 ...
 private:
  // 寄存器偏移定义
  static constexpr const uint32_t kRegDR = 0x00;       // 数据寄存器
  static constexpr const uint32_t kRegRSRECR = 0x04;   // 接收状态/错误清除
  static constexpr const uint32_t kRegFR = 0x18;       // 标志寄存器
  static constexpr const uint32_t kRegIBRD = 0x24;     // 整数波特率分频器
  static constexpr const uint32_t kRegFBRD = 0x28;     // 小数波特率分频器
  static constexpr const uint32_t kRegLCRH = 0x2C;     // 线路控制寄存器
  static constexpr const uint32_t kRegCR = 0x30;       // 控制寄存器
  static constexpr const uint32_t kRegIMSC = 0x38;     // 中断屏蔽设置/清除

  // 控制位定义
  static constexpr const uint32_t kFRTxFIFO = (1 << 5);    // 发送FIFO满
  static constexpr const uint32_t kLCRHWlen8 = (3 << 5);   // 8位数据
  static constexpr const uint32_t kCREnable = (1 << 0);    // UART使能
  static constexpr const uint32_t kCRTxEnable = (1 << 8);  // 发送使能
  static constexpr const uint32_t kCRRxEnable = (1 << 9);  // 接收使能
  static constexpr const uint32_t kIMSCRxim = (1 << 4);    // 接收中断屏蔽
};
```
...
```cpp
// 中断相关寄存器
static constexpr const uint32_t kRegRIS = 0x3C;   // 原始中断状态
static constexpr const uint32_t kRegMIS = 0x40;   // 屏蔽中断状态
static constexpr const uint32_t kRegICR = 0x44;   // 中断清除

// 中断类型
static constexpr const uint32_t kIMSCRTIM = (1 << 6);  // 接收超时中断
static constexpr const uint32_t kIMSCRxim = (1 << 4);  // 接收中断
```
