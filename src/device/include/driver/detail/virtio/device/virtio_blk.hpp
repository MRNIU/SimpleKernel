...
#ifndef SIMPLEKERNEL_SRC_DEVICE_INCLUDE_DRIVER_DETAIL_VIRTIO_DEVICE_VIRTIO_BLK_HPP_
#define SIMPLEKERNEL_SRC_DEVICE_INCLUDE_DRIVER_DETAIL_VIRTIO_DEVICE_VIRTIO_BLK_HPP_
  ...
  [[nodiscard]] auto SubmitSyncRequest(ReqType type, uint64_t sector,
    ...
    constexpr uint32_t spin_limit = [] {
      if constexpr (SpinWaitTraits<PlatformEnvironment>) {
        return static_cast<uint32_t>(PlatformEnvironment::kMaxSpinIterations);
      } else {
...
#endif /* SIMPLEKERNEL_SRC_DEVICE_INCLUDE_DRIVER_DETAIL_VIRTIO_DEVICE_VIRTIO_BLK_HPP_ \
        */
