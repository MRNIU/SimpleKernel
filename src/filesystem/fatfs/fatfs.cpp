...
FatFsFileSystem::~FatFsFileSystem() {
  ...
  auto FatFsFileSystem::Mount(vfs::BlockDevice * device)
      -> Expected<vfs::Inode*> {
    if (device == nullptr) {
  ...
  auto FatFsFileSystem::AllocateDir() -> DIR* {
    ...
    auto FatFsFileSystem::FatFsInodeOps::Lookup(
        vfs::Inode * dir, const char* name) -> Expected<vfs::Inode*> {
    ...
    auto FatFsFileSystem::FatFsInodeOps::Create(
        vfs::Inode * dir, const char* name, vfs::FileType type)
        -> Expected<vfs::Inode*> {
      auto* dir_fi = static_cast<FatInode*>(dir->fs_private);
      std::array<char, kPathBufSize> full_path{};
      strncpy(full_path.data(), dir_fi->path.data(), full_path.size() - 1);
    ...
    auto FatFsFileSystem::FatFsInodeOps::Unlink(
        vfs::Inode * dir, const char* name) -> Expected<void> {
      auto* dir_fi = static_cast<FatInode*>(dir->fs_private);
      std::array<char, kPathBufSize> full_path{};
      strncpy(full_path.data(), dir_fi->path.data(), full_path.size() - 1);
      size_t dir_len = strlen(full_path.data());
    ...
    auto FatFsFileSystem::FatFsInodeOps::Mkdir(
        vfs::Inode * dir, const char* name) -> Expected<vfs::Inode*> {
      return Create(dir, name, vfs::FileType::kDirectory);
    }

    auto FatFsFileSystem::FatFsInodeOps::Rmdir(
        vfs::Inode * dir, const char* name) -> Expected<void> {
      // f_unlink 同时处理文件和空目录
    ...
    auto FatFsFileSystem::FatFsFileOps::Write(
        vfs::File * file, const void* buf, size_t count) -> Expected<size_t> {
      auto* fi = static_cast<FatInode*>(file->inode->fs_private);
      if (fi->fil == nullptr) {
        return std::unexpected(Error{ErrorCode::kFsInvalidFd});
      }
      ...
      FRESULT fr =
          f_write(fi->fil, buf, static_cast<UINT>(count), &bytes_written);
      if (fr != FR_OK) {
    ...
    auto FatFsFileSystem::FatFsFileOps::Seek(vfs::File * file, int64_t offset,
      ...
      switch (whence) {
        ...
        case vfs::SeekWhence::kCur:
          new_pos = static_cast<FSIZE_t>(static_cast<int64_t>(f_tell(fi->fil)) +
                                         offset);
          break;
        case vfs::SeekWhence::kEnd:
          new_pos = static_cast<FSIZE_t>(static_cast<int64_t>(f_size(fi->fil)) +
                                         offset);
          break;
      }
    ...
    auto FatFsFileSystem::FatFsFileOps::Close(vfs::File * file)
        -> Expected<void> {
      auto* fi = static_cast<FatInode*>(file->inode->fs_private);
    ...
    auto FatFsFileSystem::FatFsFileOps::ReadDir(
        vfs::File * file, vfs::DirEntry * dirent, size_t count)
        -> Expected<size_t> {
      auto* fi = static_cast<FatInode*>(file->inode->fs_private);
