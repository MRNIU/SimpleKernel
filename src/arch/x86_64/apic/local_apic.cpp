...
void LocalApic::SendInitIpi(uint32_t destination_apic_id) const {
  ...
  klog::Info("INIT IPI sent to APIC ID 0x%llx",
             static_cast<uint64_t>(destination_apic_id));
}
...
void LocalApic::PrintInfo() const {
  ...
  if (is_x2apic_mode_) {
    ...
    klog::Info("APIC Base Address: 0x%llx", static_cast<uint64_t>(apic_base_));
  }
}
